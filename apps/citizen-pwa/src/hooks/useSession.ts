...
import sessionManager, { UserSession, useSession as useSessionManager } from '../lib/session';
...
export const useSession = (): SessionState => {
  ...
  const baseSession = useSessionManager();
  ...
  const updateWalletData = useCallback((walletId: string) => {
    sessionManager.updateWalletData(walletId);
  }, []);
  ...
  return {
    ...
    currentSession: baseSession.currentSession,
    isLoading,
    timeUntilExpiry,
    hasPermission: baseSession.hasPermission,
    logout: baseSession.logout,
    refreshSession,
    updateWalletData
  };
};
